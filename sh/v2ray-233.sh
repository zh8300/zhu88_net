...
path_config_ask() {
	...
	while :; do
		...
		N | n)
			echo
			echo
			echo -e "$yellow 网站伪装 和 路径分流 = $cyan不配置$none"
			echo "----------------------------------------------------------------"
			echo
			break
			;;
...
proxy_site_config() {
	...
	while :; do
		...
		echo -e "例如...你当前的域名是 $green$domain$none , 伪装的网址的是 https://liyafly.com"
		...
		*[#$]*)
			echo
			echo -e " 伪装网址不能包含$red # $none或$red $ $none这两个符号 "
			echo
			error
			;;
		*)
...
install_info() {
	...
	if [[ $v2ray_transport == [45] ]]; then
		...
		echo -e "$yellow 你的域名 = $cyan$domain$none"
		echo
		echo -e "$yellow 域名解析 = ${cyan}确定已经解析$none"
		echo
		echo -e "$yellow 自动配置 TLS = $cyan$install_caddy_info$none"
...
*)
	echo
	echo -e " 你输入的这个参数 <$red $args $none> ..."
	echo
	echo -e " 本脚本仅支持输入$green local / online $none参数"
	echo
